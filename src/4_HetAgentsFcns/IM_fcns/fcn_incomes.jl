function incomes(n_par, m_par, mcw, A, RB, τprog, τlev, H, Ht,w,N, profits, unionprofits, av_tax_rate)


   # profits        = (1.0 -mc) .* Y
    tax_prog_scale = (m_par.γ + m_par.τ_prog)/((m_par.γ + τprog))
   # unionprofits   = w * N * (1.0 - mcw)
    eff_int      = (RB .+ (m_par.Rbar .* (n_par.mesh_m.<=0.0)))  # effective rate (need to check timing below and inflation)
    ...
    inc[3][:,end].= 0.0
    inc[4][:,end].= τlev.*(n_par.mesh_y[:,end] .* profits).^(1.0-τprog) # profit income net of taxes
    ...
    incgross[3][:,end].= (n_par.mesh_y[:,end] .* profits)
